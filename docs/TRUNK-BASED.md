...
5. **Todo cambio entra a `main` mediante un PR aprobado** — sin excepciones. No se permite push directo a `main`, `lowmem` o `ultra-lowmem`, ni siquiera a administradores. El bypass de admin está prohibido por doctrina del repositorio (ver [AGENTS.md](../AGENTS.md) §2).
...
1. Al abrir un PR (o marcarlo ready_for_review), el workflow `auto-merge.yml` habilita auto-merge (squash) en el PR.
...
El bot `yap-reviewer` (`pr-review.yml`) publica un comentario con su análisis de políticas Hardness, pero **no aprueba** — `GITHUB_TOKEN` no puede emitir reviews `APPROVE` (limitación de plataforma). La aprobación es siempre humana. Ver [AGENTS.md](../AGENTS.md) §3.
