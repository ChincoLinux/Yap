...
1. **Cambios de código**: PR revisado y aprobado por `core-devs` antes de merge. **Todo cambio** —sin importar tamaño— pasa por PR. Sin bypass de admin (ver [AGENTS.md](AGENTS.md) §2).
...
- `main` está cubierto por branch protection que exige **PR + CI verde + 1 approval**. **Sin bypass de admin** (ver [AGENTS.md](AGENTS.md) §2). **Overlay trunk-based de Yap** ([docs/TRUNK-BASED.md](docs/TRUNK-BASED.md)): ramas cortas e integración frecuente; todo cambio entra vía PR aprobado, sin importar el tamaño o el rol del autor.
