...
Adapted from os-santiago/homedir for ChincoLinux/Yap (Python).
Verifies that every PR has traceability to an issue, per A-Dev rule #30:
"every implementation PR must reference its source issue".
"""
...
def check_pr(github_token: str, repository: str, pr_number: int) -> tuple:
    ...
    msg = (
        ...
        "Traceability is required per A-Dev rule #30 and AGENTS.md §2."
    )
    return ("false", msg, [])
