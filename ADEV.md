...
- This copy is the canonical organization-level doctrine for all ChincoLinux repositories. Per-repository `ADEV.md` files point to this copy and add only repository-specific overlays.
- Content contributed to the community is bilingual: Spanish primary, English mirror when localization is required.
- Teams: `core-devs` (maintainers) and `contributors` (active collaborators). See `GOVERNANCE.md`.
- Validation targets for the ChincoLinux ecosystem: repositories under the ChincoLinux organization, their CI workflows, and the community site.
- Keep this file synchronized with the upstream source of truth at `https://github.com/scanalesespinoza/adev`.
