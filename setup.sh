...
echo "    • $WHITELIST_DIR/dirs.conf  — Directorios permitidos para abrir archivos"
...
sudo cp "$SCRIPT_DIR/whitelist/dirs.conf" "$WHITELIST_DIR/"
...
if [ -f "$SCRIPT_DIR/yap-agent.md" ]; then
  ...
  echo "  • yap-agent.md no existe en el repo — saltando copia (no es error)"
fi
